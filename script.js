...
// ==========================================================================
// 12. 3D MEMORY UNIVERSE: 360° ORBITAL SPATIAL INSTALLATION ENGINE
// ==========================================================================
const UNI_CONFIG = {
  total: 12,
  stepDeg: 30, // 360 / 12
...
const UNI_TITLES = [
  ...
  "Three Flowers, One Frame",
  "The Classic Monochrome"
];

function initMemoryUniverse3D() {
  const stage = document.getElementById('universeStage3D');
  const ring = document.getElementById('memoryRing3D');
  const panels = document.querySelectorAll('.memory-panel-3d');
  const dotsBar = document.getElementById('universeDotsBar');
  ...
  function updateRadius() {
    ...
    if (w < 480) {
      UNI_CONFIG.radius = 300;
    } else if (w < 768) {
      UNI_CONFIG.radius = 420;
    } else if (w < 1200) {
      UNI_CONFIG.radius = 540;
    } else {
      UNI_CONFIG.radius = 640;
    }
  }
  updateRadius();
  ...
  // Generate 12 indicator dots
  if (dotsBar) {
    dotsBar.innerHTML = '';
    for (let i = 0; i < UNI_CONFIG.total; i++) {
      const dot = document.createElement('button');
      dot.type = 'button';
      dot.className = `uni-dot ${i === 0 ? 'active' : ''}`;
      dot.setAttribute('aria-label', `Jump to memory ${i + 1}`);
  ...
  stage.addEventListener('pointerdown', (e) => {
    uniState.isDragging = true;
    uniState.startX = e.clientX;
    uniState.lastX = e.clientX;
    uniState.startRotY = uniState.rotationY;
    uniState.velocity = 0;
    stage.setPointerCapture(e.pointerId);
    if (typeof gsap !== 'undefined') gsap.killTweensOf(uniState);
  });

  stage.addEventListener('pointermove', (e) => {
    if (!uniState.isDragging) return;
    const deltaX = e.clientX - uniState.startX;
    uniState.velocity = (e.clientX - uniState.lastX) * 0.3;
    uniState.lastX = e.clientX;
    uniState.rotationY = uniState.startRotY + deltaX * UNI_CONFIG.sensitivity;
    renderUniverseFrame();
  });

  function onPointerEnd(e) {
    if (!uniState.isDragging) return;
    uniState.isDragging = false;
    try { stage.releasePointerCapture(e.pointerId); } catch(err) {}

    // Snap to nearest memory with inertia
    const projectedRot = uniState.rotationY + uniState.velocity * 4;
    const nearestIndex = Math.round(-projectedRot / UNI_CONFIG.stepDeg);
    const snapRotY = -nearestIndex * UNI_CONFIG.stepDeg;
    ...
    if (typeof gsap !== 'undefined') {
      gsap.to(uniState, {
        ...
        duration: 0.75,
        ease: "power2.out",
        onUpdate: renderUniverseFrame,
        onComplete: updateUniverseUI
      });
    } else {
      uniState.rotationY = snapRotY;
      renderUniverseFrame();
      updateUniverseUI();
    }
  }
  ...
  stage.addEventListener('wheel', (e) => {
    if (Math.abs(e.deltaX) > Math.abs(e.deltaY) || e.shiftKey) {
      ...
      wheelTimeout = setTimeout(() => {
        rotateUniverseStep(dir);
      }, 40);
    }
  }, { passive: false });

  // Hover detection for subtle ambient drift
  stage.addEventListener('mouseenter', () => { uniState.isHovered = true; });
  stage.addEventListener('mouseleave', () => { uniState.isHovered = false; });

  // Click handler on panels
  panels.forEach((panel) => {
    panel.addEventListener('click', (e) => {
      // If clicking zoom overlay or photo of active card, let it open lightbox
      if (e.target.closest('.panel-zoom-overlay')) return;

      const idx = parseInt(panel.getAttribute('data-index'), 10);
      if (idx !== uniState.activeIndex) {
        e.stopPropagation();
        goToMemory(idx);
      }
    });
  ...
  window.addEventListener('keydown', (e) => {
    ...
    const inView = stageRect.top < window.innerHeight && stageRect.bottom > 0;
    if (!inView) return;

    if (e.key === 'ArrowRight') {
      rotateUniverseStep(1);
  ...
  function ambientOrbitLoop(now) {
    const delta = now - lastTime;
    lastTime = now;
    if (!uniState.isDragging && !uniState.isHovered && delta < 100) {
      // Very gentle celestial drift (0.015 deg per frame)
      uniState.rotationY -= 0.012;
      renderUniverseFrame();
    }
    requestAnimationFrame(ambientOrbitLoop);
  }
  requestAnimationFrame(ambientOrbitLoop);
}

function renderUniverseFrame() {
  const ring = document.getElementById('memoryRing3D');
  const centerpiece = document.getElementById('centerpieceCore');
  const panels = document.querySelectorAll('.memory-panel-3d');
  if (!ring || !panels.length) return;

  const R = UNI_CONFIG.radius;
  const rotY = uniState.rotationY;

  // Rotate ring around center Y-axis
  ring.style.transform = `rotateY(${rotY}deg)`;

  // Keep centerpiece facing viewer smoothly
  if (centerpiece) {
    centerpiece.style.transform = `rotateY(${-rotY * 0.2}deg)`;
  }

  // Calculate each panel's relative angle to determine depth
  let closestDist = Infinity;
  let closestIndex = 0;

  panels.forEach((panel, i) => {
    const baseAngle = i * UNI_CONFIG.stepDeg;
    // Calculate effective angle from front viewer (-180 to 180)
    let relAngle = ((baseAngle + rotY) % 360 + 540) % 360 - 180;
    ...
    // Position panel in 3D circular ring
    panel.style.transform = `rotateY(${baseAngle}deg) translateZ(${R}px)`;

    // Depth-of-field visual effects based on distance from viewer
    const panelInner = panel.querySelector('.panel-inner-card');
    if (absAngle < 20) {
      // Active front memory
      panel.style.zIndex = 35;
      panel.style.opacity = '1';
      panel.style.filter = 'blur(0px) brightness(1.08)';
      if (panelInner) panelInner.style.transform = 'scale(1.05)';
      panel.classList.add('active-memory');
    } else if (absAngle < 55) {
      panel.style.zIndex = 20;
      panel.style.opacity = '0.78';
      panel.style.filter = 'blur(0.5px) brightness(0.95)';
      if (panelInner) panelInner.style.transform = 'scale(0.92)';
      panel.classList.remove('active-memory');
    } else if (absAngle < 90) {
      panel.style.zIndex = 10;
      panel.style.opacity = '0.45';
      panel.style.filter = 'blur(2px) brightness(0.8)';
      if (panelInner) panelInner.style.transform = 'scale(0.78)';
      panel.classList.remove('active-memory');
    } else {
      // Rear facing memories
      panel.style.zIndex = 2;
      panel.style.opacity = '0.18';
      panel.style.filter = 'blur(4px) brightness(0.6)';
      if (panelInner) panelInner.style.transform = 'scale(0.65)';
      panel.classList.remove('active-memory');
    }
  });

  if (uniState.activeIndex !== closestIndex) {
    uniState.activeIndex = closestIndex;
    updateUniverseUI();
  }
}

function updateUniverseUI() {
  const activeIdx = uniState.activeIndex;
  const counterEl = document.getElementById('uniCounter');
  const nameEl = document.getElementById('uniActiveName');
  const dots = document.querySelectorAll('.uni-dot');

  const displayNum = String(activeIdx + 1).padStart(2, '0');
  if (counterEl) counterEl.textContent = `${displayNum} / 12`;
  if (nameEl) nameEl.textContent = UNI_TITLES[activeIdx] || `Memory ${displayNum}`;
...
function rotateUniverseStep(direction) {
  const targetIdx = (uniState.activeIndex + direction + UNI_CONFIG.total) % UNI_CONFIG.total;
  goToMemory(targetIdx);
}

function goToMemory(targetIndex) {
  if (targetIndex < 0 || targetIndex >= UNI_CONFIG.total) return;

  const currentRot = uniState.rotationY;
  const targetBaseAngle = targetIndex * UNI_CONFIG.stepDeg;

  // Find shortest rotational path
  let currentNormalized = ((currentRot % 360) + 360) % 360;
  let targetAngle = -targetBaseAngle;
  let diff = targetAngle - currentRot;
  diff = ((diff + 180) % 360 + 360) % 360 - 180;
  const finalRotY = currentRot + diff;
  ...
  if (typeof gsap !== 'undefined') {
    gsap.to(uniState, {
      ...
      onUpdate: renderUniverseFrame,
      onComplete: () => {
        uniState.activeIndex = targetIndex;
        updateUniverseUI();
      }
    });
  } else {
    uniState.rotationY = finalRotY;
    uniState.activeIndex = targetIndex;
    renderUniverseFrame();
    updateUniverseUI();
  }
